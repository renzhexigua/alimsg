...
'''
Created on 2012-7-3
Modified on 2016-01-25 for compatability(py3)
...
SYSTEM_GENERATE_VERSION = "taobao-sdk-python-20160122"
...
def sign(secret, parameters):
    ...
    if hasattr(parameters, "items"):
        ...
        parameters = "%s%s%s" % (secret,
                                 ...
                                 secret)
    sign = hashlib.md5(parameters.encode(encoding="utf-8")).hexdigest().upper()
    return sign
...
class RestApi(object):
    ...
    def getResponse(self, authrize=None, timeout=30):
        ...
        connection = httplib.HTTPConnection(self.__domain.split("/")[0], str(self.__port), timeout)
        # compatability for 3.4
        # Issue https://github.com/jgarzik/python-bitcoinrpc/pull/34
        # fix https://github.com/4tar/python-bitcoinrpc/commit/e43303bba7a671b383016d29f55fd03aa0df235b
        sys_parameters = {
            P_FORMAT: 'json',
        ...
        if "error_response" in jsonobj.keys():
            error = TopException()
            if P_CODE in jsonobj["error_response"].keys():
                error.errorcode = jsonobj["error_response"][P_CODE]
            if P_MSG in jsonobj["error_response"].keys():
                error.message = jsonobj["error_response"][P_MSG]
            if P_SUB_CODE in jsonobj["error_response"].keys():
                error.subcode = jsonobj["error_response"][P_SUB_CODE]
            if P_SUB_MSG in jsonobj["error_response"].keys():
